...
from objects.chapter_parser import (
    parse_chapters_from_ai_response,
    validate_chapter_character_names,
    parse_single_chapter_from_ai_response,
)
    ...
    if story_data:
        ...
        if selected_plot_line_data and isinstance(selected_plot_line_data, dict):
            if "name" in selected_plot_line_data and "plotline" in selected_plot_line_data:
                plot_line = PlotLine(name=selected_plot_line_data["name"], plotline=selected_plot_line_data["plotline"])
        ...
        if "story_data" not in session:
            minimal_story_data = {
                ...
                "secondary_archetypes": (
                    [archetype.value for archetype in story.secondary_archetypes] if story.secondary_archetypes else []
                ),
            }
...
def protagonist_archetype_selection():
    ...
    if (
        not story.story_type_name
        or not story.subtype_name
        or not story.genre
        or not story.sub_genre
        or not story.writing_style
    ):
...
def secondary_archetype_selection():
    ...
    if (
        ...
        or not story.sub_genre
        or not story.writing_style
        or not story.protagonist_archetype
    ):
        flash(
            "Please complete story type, genre, sub-genre, writing style, and protagonist archetype selection first.",
            "error",
        )
        return redirect(url_for("index"))
...
def select_plot_line():
    ...
    try:
        ...
        if not data or "name" not in data or "plotline" not in data:
            return jsonify({"success": False, "error": "Invalid plot line data provided"}), 400
        ...
        if story.set_selected_plot_line(plot_line):
            ...
            return jsonify({"success": False, "error": "Failed to set selected plot line"}), 400
...
def generate_chapter(chapter_number):
    ...
    try:
        ...
        if not generated_chapter:
            return jsonify({"success": False, "error": "Failed to parse chapter from AI response"})
    ...
    chapters = [
        Chapter(
            ...
            character_impact=[
                {
                    "character": "Lyra",
                    "effect": "Discovers her identity as the Chosen One and reluctantly accepts her destiny, feeling overwhelmed but determined.",
                },
                {
                    "character": "Aldric",
                    "effect": "Reveals long-held secrets about the prophecy and begins mentoring Lyra, finding purpose in his centuries of waiting.",
                },
            ],
            point_of_view="Lyra",
        ...
        Chapter(
            ...
            character_impact=[
                {
                    "character": "Lyra",
                    "effect": "Sees the true scope of the threat and realizes the urgency of her mission, beginning to overcome her self-doubt.",
                },
                {
                    "character": "Aldric",
                    "effect": "Shares more of his burden and knowledge while training Lyra in basic magic and survival skills.",
                },
            ],
            point_of_view="Lyra",
        ...
        Chapter(
            ...
            character_impact=[
                {
                    "character": "Lyra",
                    "effect": "Learns to trust others and begins developing leadership skills, though still struggling with confidence.",
                },
                {
                    "character": "Kael",
                    "effect": "Finds new purpose in the quest and begins to believe in redemption through service to others.",
                },
            ],
            point_of_view="Kael",
